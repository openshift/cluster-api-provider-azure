...
func customizeDeployments(obj *unstructured.Unstructured) {
	...
	for i := range deployment.Spec.Template.Spec.Containers {
		...
		if !strings.HasPrefix(container.Image, "registry.ci.openshift.org") && !isWhitelistedImage(container.Image) {
...
// TODO OCPCLOUD-2642 - Keep the upstream image until we have OCP image.
func isWhitelistedImage(image string) bool {
	return strings.HasPrefix(image, "mcr.microsoft.com/k8s/azureserviceoperator")
}
