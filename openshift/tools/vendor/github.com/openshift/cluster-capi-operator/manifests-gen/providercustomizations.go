...
func azureCustomizations(obj *unstructured.Unstructured) {
	...
	case "Deployment":
		...
		case "azureserviceoperator-controller-manager":
			asoDeploymentCustomizations(deployment)
		case "capz-controller-manager":
			capzDeploymentCustomizations(deployment)
		}
...
func asoDeploymentCustomizations(deployment *appsv1.Deployment) {
		...
		if container.Name == "manager" {
			for j := range container.Env {
				...
				case "AZURE_SUBSCRIPTION_ID":
					...
					env.ValueFrom.SecretKeyRef.Name = "capz-manager-bootstrap-credentials"
...
func capzDeploymentCustomizations(deployment *appsv1.Deployment) {
	// Modify bootstrap secret keys as they don't match with what is created by CCO.
	for i := range deployment.Spec.Template.Spec.Containers {
		container := &deployment.Spec.Template.Spec.Containers[i]
		if container.Name == "manager" {
