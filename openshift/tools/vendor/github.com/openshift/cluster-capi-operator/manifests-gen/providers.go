...
func (p *provider) compressToZstd(data []byte) (bytes.Buffer, error) {
	var compressed bytes.Buffer

	writer, err := zstd.NewWriter(&compressed, zstd.WithEncoderLevel(zstd.EncoderLevelFromZstd(19)))
	if err != nil {
...
func (p *provider) writeProviderCustomizedComponents(resourceMap map[resourceKey][]unstructured.Unstructured) error {
	...
	if err != nil {
		return fmt.Errorf("failed to convert unstructure object to YAML: %w", err)
	}
	...
	return os.WriteFile(path.Join(*basePath, "openshift", "infrastructure-components-openshift.yaml"), ensureNewLine(combined), 0600)
}
...
func (p *provider) writeProviderComponentsConfigmap(fileName string, objs []unstructured.Unstructured) error {
	...
	compressed, err := p.compressToZstd(combined)
	if err != nil {
		return fmt.Errorf("failed to compress components: %w", err)
	}
...
func importProvider(p provider) error {
	...
	if err := p.writeProviderCustomizedComponents(resourceMap); err != nil {
		return fmt.Errorf("error writing infrastructure-components-openshift.yaml: %w", err)
	}
